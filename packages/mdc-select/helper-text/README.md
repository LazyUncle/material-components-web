...
`mdc-select-helper-text--persistent-validation` | When the helper text is serving as a validation message, make it permanently visible regardless of the select's validity.
...
`setPersistentValidation(isValidation: boolean) => void` | Sets the validation helper text to show even if the select is valid.
